...
const AuthManager: React.FC = () => {
  ...
  if (!initialLoadComplete) {
    return <Button variant="outline" size="sm" className="glass-effect border-white/20 shadow-sm transition-all duration-300">
  ...
  if (user) {
    return <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" size="sm" className="glass-effect border-white/20 shadow-sm hover:shadow-md transition-all duration-300 flex items-center" disabled={isLoading}>
  ...
  return <div>
      <Button variant="outline" size="sm" className="glass-effect border-white/20 shadow-sm hover:shadow-md transition-all duration-300 flex items-center" onClick={handleLoginClick} aria-label="Login / Register">
      ...
      <Dialog open={isAuthModalOpen} onOpenChange={setIsAuthModalOpen}>
        <DialogContent className="sm:max-w-md animate-scale-in">
          {isLoginView ? <LoginForm onSuccess={handleAuthSuccess} onRegisterClick={() => setIsLoginView(false)} /> : <RegisterForm onSuccess={handleAuthSuccess} onLoginClick={() => setIsLoginView(true)} />}
        </DialogContent>
      </Dialog>
    </div>;
};
