...
const QuizGenerator: React.FC<QuizGeneratorProps> = ({ initialTopic = "" }) => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const questions = await generateQuestions(combinedObjectives, {
        difficulty,
        count: questionCount,
        questionTypes
      });
  ...
  const handleFileTextExtracted = (text: string) => {
    ...
    toast.success("Document processed with RAG. The system will use this content to enhance quiz questions.");
  };
