...
export async function generateQuestions(
  ...
  try {
    ...
    toast.loading("Generating practice questions with AI...");
    ...
    console.log("Retrieved RAG context:", relevantContext.substring(0, 100) + "...");
    
    // Create a combined prompt with RAG context
    let promptWithContext = learningObjectives;
    
    if (relevantContext && relevantContext !== "No relevant information found in the uploaded documents.") {
      promptWithContext = `Learning objectives: ${learningObjectives}\n\nContext from relevant documents:\n${relevantContext}\n\nGenerate questions based on this context and learning objectives.`;
    }
    
    // Customize the system prompt based on options
    const systemPrompt = `You are a quiz generator. Create ${count} practice questions (${multipleChoiceCount} multiple choice and ${fillInCount} fill-in-the-blank) based on the provided learning objectives and context. Difficulty level should be ${difficulty}. Return your response in JSON format with the following structure: {"questions": [{"id": "q1", "type": "multiple_choice", "question": "Question text", "options": ["Option A", "Option B", "Option C", "Option D"], "correctAnswer": 0, "explanation": "Explanation", "difficulty": "${difficulty}"}, {"id": "q2", "type": "fill_in", "question": "Question with blank ________.", "correctAnswer": "answer", "explanation": "Explanation", "difficulty": "${difficulty}"}]}`;
    ...
    const response = await fetch("https://api.deepseek.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Create a test based on these learning objectives: ${promptWithContext}`
          }
        ],
        temperature: 0.7,
        max_tokens: 2000
